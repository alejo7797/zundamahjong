...
async def set_player(sid: str, player: Player) -> None:
    with player_sid_lock:
        ...
        if old_player:
            ...
            await sio.close_room(old_player.id)
        id_to_sid[player.id] = sid
        sid_to_player[sid] = player
        await sio.enter_room(sid, player.id)


async def unset_player(sid: str) -> None:
    with player_sid_lock:
        player = sid_to_player.get(sid)
        if player:
            id_to_sid.pop(player.id)
            sid_to_player.pop(sid)
            await sio.close_room(player.id)
