...
@sio_on("connect")
async def connect(sid: str, environ: dict[str, Any], auth: object = None) -> None:  # pyright: ignore[reportExplicitAny]
...
async def disconnect(sid: str, reason: str) -> None:
    ...
    if player is None:
        ...
        await GameRoom.try_disconnect(player)
    await unset_player(sid)


@sio_on("action")
async def handle_action(sid: str, action_data: object, history_index: object) -> None:
    ...
    await game_room.game_controller.submit_action(player, action, history_index)


@sio_on("next_round")
async def start_next_round(sid: str) -> None:
    player = get_player(sid)
    game_room = GameRoom.get_player_room(player)
    if game_room is None:
        raise Exception("Player is not in a game room!")
    ...
    if not game_room.game_controller.game.is_game_end:
        await game_room.game_controller.start_next_round(player)
    else:
        await game_room.end_game()


@sio_on("set_name")
async def on_set_name(sid: str, name: object, password: object) -> None:
    ...
    await set_player(sid, player)
    if player.has_account and player.new_user:
        await sio.emit_info("Account successfully created.", to=sid)
    await sio.emit("player_info", player.model_dump(), to=sid)
    game_room = await GameRoom.try_reconnect(player)
    if game_room is not None and game_room.game_controller is not None:
        await game_room.game_controller.emit_info(player)


@sio_on("unset_name")
async def on_unset_name(sid: str) -> None:
    await GameRoom.try_disconnect(get_player(sid))
    await unset_player(sid)
    await sio.emit("unset_name", to=sid)


@sio_on("change_password")
async def on_change_password(
    sid: str, cur_password: object, new_password: object
) -> None:
    ...
    await sio.emit_info("Password changed successfully.", to=sid)


@sio_on("get_rooms")
async def on_get_rooms(sid: str) -> None:
    await GameRoom.emit_rooms_list(sid)


@sio_on("create_room")
async def on_create_room(sid: str, room_name: object, player_count: object) -> None:
    ...
    await GameRoom.create_room(get_player(sid), room_name, player_count)


@sio_on("join_room")
async def on_join_room(sid: str, room_name: object) -> None:
    if not isinstance(room_name, str):
        raise Exception("Argument room_name is not a string!")
    GameRoom.verify_room_name(room_name)
    await GameRoom.join_room(get_player(sid), room_name)


@sio_on("leave_room")
async def on_leave_room(sid: str) -> None:
    await GameRoom.leave_room(get_player(sid))


@sio_on("set_avatar")
async def on_set_avatar(sid: str, avatar_code: object) -> None:
    if not isinstance(avatar_code, int):
        raise Exception("Avatar code is not an integer!")
    avatar = Avatar(avatar_code)
    await GameRoom.set_avatar(get_player(sid), avatar)


@sio_on("game_options")
async def on_game_options(sid: str, game_options_data: object) -> None:
    game_options = GameOptions.model_validate(game_options_data)
    await GameRoom.set_game_options(get_player(sid), game_options)
