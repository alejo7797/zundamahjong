...
class GameRoom:
    ...
    @classmethod
    async def emit_rooms_list(cls, sid: str) -> None:
        await sio.emit(
            "rooms_info",
    ...
    async def create_room(
        ...
        await game_room.broadcast_room_info()
        return game_room

    @classmethod
    async def join_room(cls, player: Player, room_name: str) -> GameRoom:
        with rooms_lock:
    ...
    @classmethod
    async def leave_room(cls, player: Player) -> GameRoom:
        with rooms_lock:
            try:
            ...
            await sio.emit("room_info", None, to=player.id)
            await game_room.broadcast_room_info()
        return game_room

    async def broadcast_room_info(self) -> None:
        async with asyncio.TaskGroup() as tg:
            for player in self.joined_players:
                tg.create_task(
                    sio.emit(
                        "room_info", self.room_detailed_info.model_dump(), to=player.id
                    )
                )

    async def broadcast_game_end(self) -> None:
        async with asyncio.TaskGroup() as tg:
            for player in self.joined_players:
                tg.create_task(sio.emit("info", None, to=player.id))
    ...
    @classmethod
    async def try_disconnect(cls, player: Player) -> None:
        with rooms_lock:
    ...
    @classmethod
    async def try_reconnect(cls, player: Player) -> GameRoom | None:
        with rooms_lock:
            game_room = player_rooms.get(player.id)
    ...
    @classmethod
    async def set_avatar(cls, player: Player, avatar: Avatar) -> None:
    ...
    @classmethod
    async def set_game_options(cls, player: Player, game_options: GameOptions) -> None:
        game_room = cls.get_player_room(player)
        if game_room is None:
            raise Exception("Player is not in a room!")
    ...
    async def start_game(self) -> None:
        ...
        with rooms_lock:
            ...
            self._save_avatars()
            self.game_controller = await GameController.create(
