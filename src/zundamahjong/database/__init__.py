...
def _app_ctx_id() -> int:
    """Get the id of the current Quart application context object."""
...
class SQLAlchemy:
    ...
    def session(self) -> Session:
        if not self._session.registry.has():
            logger.info(
                f"Opening database session within Quart application context {_app_ctx_id()}"
            )
    ...
    def _close(self, exc: BaseException | None) -> None:
        if self._session.registry.has():
            logger.info(
                f"Closing database session within Quart application context {_app_ctx_id()}"
            )
